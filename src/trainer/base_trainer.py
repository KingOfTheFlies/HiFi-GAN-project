...
class BaseTrainer:
        ...
        for batch_idx, batch in enumerate(
            ...
            if batch_idx % self.log_step == 0:
                ...
                self.writer.add_scalar(
                    "Generator learning rate", self.generator_scheduler.get_last_lr()[0]
                )
                self.writer.add_scalar(
                    "Discriminator learning rate", self.discriminator_scheduler.get_last_lr()[0]
                )
    ...
    def _resume_checkpoint(self, resume_path):
        ...
        if (
            ...
            or checkpoint["config"]["generator_scheduler"] != self.config["generator_scheduler"]) and
            (checkpoint["config"]["discriminator_optimizer"] != self.config["discriminator_optimizer"]
            or checkpoint["config"]["discriminator_scheduler"] != self.config["discriminator_scheduler"])
        ):
            self.logger.warning(
